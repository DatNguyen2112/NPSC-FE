...
import { InvestorFacade, InvestorViewModel } from '@store';
...
interface DataType extends InvestorViewModel {
  stt?: number;
  key?: string;
...
const Page: React.FC = () => {
  ...
  const dataSource: DataType[] =
    investorFacade.pagination?.content.map(
      (items, index): DataType => ({
        ...items,
        stt:
          (Number(investorFacade.pagination?.page ?? 0) - 1) * Number(investorFacade.pagination?.size ?? 0) + index + 1,
        key: uuidv4(),
        id: items.id ?? '-',
        code: items.code ?? '-',
        createdOnDate: dayjs(items?.createdOnDate).format('DD/MM/YYYY'),
        investorTypeName: items?.investorType?.name,
      }),
    ) ?? [];
      ...
      render: (_, record: any) => (
        <Space>
          {!rightMapRoleFacade.rightDatas?.[0]?.rightCodes?.includes('UPDATE') ? (
            <Tooltip title="Bạn không có quyền thực hiện thao tác">
              <Button
                disabled={true}
                onClick={() => {
                  investorFacade.set({
                  ...
                  investorFacade.getById({ id: record.id });
  ...
  const tool = (
    <Space size={'middle'} className={'mx-4'}>
      ...
      <Button icon={<ReloadOutlined />} loading={investorFacade.isLoading} onClick={() => onChangeDataTable({})}>
        Tải lại
      </Button>
    ...
    <>
      ...
      <div className={'w-[1200px] m-auto p-3'}>
        <Flex className="px-5 mt-5" vertical gap={1.5}>
          <Spin spinning={investorFacade.isLoading}>
            <Table
              // size="small"
              ...
              //   current: investorFacade?.pagination?.page,
              //   pageSize: investorFacade?.pagination?.size,
              //   total: investorFacade?.pagination?.totalElements,
              //   pageSizeOptions: [20, 40, 60, 80],
              //   showTotal: (total, range) => `Từ ${range[0]} đến ${range[1]} trên tổng ${total}`,
              //   onChange: (page, size) => {
              //     let query = investorFacade.query;
