import {PlusOutlined, ReloadOutlined, InfoCircleOutlined, FilterFilled, CloseOutlined} from '@ant-design/icons';
...
const Page: React.FC = () => {
  ...
  const menuRef = useRef<string>('PROCESS_TEMPLATE');
  ...
  if (!parsedFilter.type) {
    parsedFilter.type = 'PROCESS_TEMPLATE';
  }
  ...
  useEffect(() => {
    ...
    codeTypeFacade.getProcessTemplate({size: -1})
  }, []);
  ...
  const dataSource: DataType[] =
    codeTypeFacade.processTemplateData?.content.map(
  ...
  const onChangeDataTable = (props: { query?: QueryParams; setKeyState?: object }) => {
    ...
    codeTypeFacade.getProcessTemplate(fillQuery);
    navigate(
