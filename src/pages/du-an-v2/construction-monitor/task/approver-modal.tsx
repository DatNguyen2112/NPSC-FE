...
function TaskApproverModal({
  ...
  useEffect(() => {
    ...
    return () => {
      taskExecutor.set({
        ...
        isChooseUserManyApproverModal: false,
  ...
  const handleChooseFulfilled = () => {
    taskExecutor.set({ isChooseUserManyApproverModal: false, listApprover: taskExecutor?.listPresent, autoSubmit });
  };
  ...
  return (
    <Modal
      width={500}
      height={700}
      open={taskExecutor.isChooseUserManyApproverModal}
      title={title}
      onCancel={() => taskExecutor.set({ isChooseUserManyApproverModal: false })}
      footer={
      ...
      <div className="mt-2 h-[360px] overflow-y-auto miniScroll">
        {Object.keys(groupByDepartmentAndTeam(listUser)).length > 0 ? (
          ...
          <Empty
            ...
            className="h-4"
            description="Chưa có nhân sự tham gia trong dự án"
          />
        )}
      </div>
    </Modal>
...
export default TaskApproverModal;
