...
function ListExecutorModal({
  ...
  useEffect(() => {
    ...
    return () => {
      taskExecutor.set({
        ...
        isChooseUserManySubTaskModal: false,
  ...
  const handleChooseFulfilled = () => {
    if (!taskExecutor?.listSubTasksExecutor) taskExecutor.listSubTasksExecutor = [];
    if (taskExecutor?.listSubTasksExecutor?.length <= indexSubTask)
      taskExecutor.listSubTasksExecutor = [
        ...taskExecutor.listSubTasksExecutor,
        ...new Array(indexSubTask + 1 - taskExecutor.listSubTasksExecutor.length).fill(undefined), // Điền các phần tử trống
      ];
    taskExecutor.set({
      isChooseUserManySubTaskModal: false,
      listSubTasksExecutor: taskExecutor?.listSubTasksExecutor.map((executor: any, idx: any) =>
        idx === indexSubTask ? taskExecutor?.listPresent : executor,
      ),
      autoSubmit,
    });
  };
  ...
  return (
    <Modal
      width={500}
      height={700}
      open={taskExecutor.isChooseUserManySubTaskModal}
      title={title}
      onCancel={() => taskExecutor.set({ isChooseUserManySubTaskModal: false })}
      footer={
      ...
      <div className="mt-2 h-[360px] overflow-y-auto miniScroll">
        {Object.keys(groupByDepartmentAndTeam(listUser)).length > 0 ? (
          ...
          <Empty
            ...
            className="h-4"
            description="Chưa có nhân sự thực hiện trong công việc này"
          />
        )}
      </div>
    </Modal>
...
export default ListExecutorModal;
