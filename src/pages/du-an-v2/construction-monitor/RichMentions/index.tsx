...
import {
  ConstructionFacade,
  CustomerServiceCommentFacade,
  CustomerServiceCommentModel,
...
function EditorCustom({ constructionId }: { constructionId: any }) {
  ...
  const customerServiceCommentFacade = CustomerServiceCommentFacade();
  const { user } = GlobalFacade();
  // const [disable, setDisable] = useState(false);

  useEffect(() => {
    customerServiceCommentFacade.get({
      size: -1,
  ...
  useEffect(() => {
    if (constructionFacade.data?.executionTeams) {
      ...
      setListUser(handleRemoveDuplicate(constructionFacade.data?.executionTeams));
    }
  }, [constructionFacade.data?.executionTeams]);

  useEffect(() => {
    switch (customerServiceCommentFacade.status) {
  ...
  }, [customerServiceCommentFacade.status]);
  ...
  const handleDelete = (commentId: string) => {
    modalApi.confirm({
      ...
      onOk: () => {
        customerServiceCommentFacade.delete(commentId);
      },
      onCancel: () => {},
      cancelText: 'Huỷ bỏ',
      okText: 'Xác nhận',
  ...
  const onSubmit = () => {
    const values: CustomerServiceCommentModel = {
      content: content,
      constructionId: constructionId,
      tagIds: taggedUsers?.map((item: any) => item?.id),
    };

    customerServiceCommentFacade.post(values);
  };
  ...
  return (
    ...
    >
      ...
      <div className={'mt-4 h-[376px] mb-2 overflow-y-auto miniScroll'}>
        {customerServiceCommentFacade.pagination?.content &&
        customerServiceCommentFacade.pagination?.content?.length > 0 ? (
          customerServiceCommentFacade?.pagination?.content?.map(
            (item: CustomerServiceCommentModel | any, index: number) => {
              return (
