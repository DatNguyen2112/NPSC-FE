import { DeleteOutlined, PlusOutlined, ReloadOutlined } from '@ant-design/icons';
import { SubHeader } from '@layouts/admin';
import { QueryParams } from '@models';
import { SearchWidget } from '@pages/shared-directory/search-widget';
import { EStatusLoaiXe, LoaiXeFacade, LoaiXeModel } from '@store';
import { uuidv4 } from '@utils';
import { Button, FormInstance, Modal, Pagination, Space, Spin, Table, Tooltip } from 'antd';
import { ColumnsType } from 'antd/es/table';
import { useEffect, useRef } from 'react';
import { useNavigate } from 'react-router';
import { useSearchParams } from 'react-router-dom';
import { RightMapRoleFacade } from 'src/store/right-map-role';
import { QuanLyLoaiXeDetails } from './loai-xe.details';
import { LoaiXeForm } from './loai-xe.form';

interface DataType extends LoaiXeModel {
  stt: number;
  key?: string;
}
let fillQuery: QueryParams;

const QuanLyLoaiXeScreen = () => {
  const loaiXeFacade = LoaiXeFacade();
  const rightMapRoleFacade = RightMapRoleFacade();
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate();
  const formRef = useRef<FormInstance | undefined>(undefined);
  const filter = searchParams.get('filter');
  const page = searchParams.get('page');
  const size = searchParams.get('size');
  const sort = searchParams.get('sort');
  const [modalApi, contextHolder] = Modal.useModal();

  useEffect(() => {
    loaiXeFacade.get({});
    rightMapRoleFacade.getRightMapByListCode('CODETYPE');
  }, []);

  const rowSelection = {
    onChange: (selectedRowKeys: string[]) => {
      loaiXeFacade.set({ selectedRowKeys });
    },
    getCheckboxProps: (record: any) => ({
      title: record.name,
    }),
    columnWidth: 30,
  };
  useEffect(() => {
    if (loaiXeFacade.status === EStatusLoaiXe.deleteManyFulfilled) {
      loaiXeFacade.set({ selectedRowKeys: [] });
      if (filter) {
        loaiXeFacade.get({ filter: filter });
      } else loaiXeFacade.get({});
    }
  }, [loaiXeFacade.status]);

  const handleRemoveSelected = () => {
    modalApi.confirm({
      title: 'Xóa tất cả loại xe vừa chọn',
      content: 'Mọi dữ liệu về những loại xe này sẽ bị xóa vĩnh viễn. Bạn có chắc muốn xóa những loại xe này?',
      onOk: () => {
        loaiXeFacade.deleteMany(loaiXeFacade.selectedRowKeys ?? []);
        setTimeout(() => {
          loaiXeFacade.get({});
        }, 500);
      },
      onCancel: () => {},
      cancelText: 'Hủy bỏ',
      okText: 'Xác nhận',
    });
  };

  const onChangeSearch = (value: string) => {
    onChangeDataTable({
      query: {
        page: 1,
        size: 20,
        filter: JSON.stringify({ FullTextSearch: value }),
      },
    });
  };

  const onChangeDataTable = (props: { query?: QueryParams; setKeyState?: object }) => {
    if (!props.query) {
      props.query = {
        page: Number(page),
        size: Number(size),
        filter: filter ?? '',
        sort: sort ?? '',
      };
    }

    fillQuery = { ...loaiXeFacade.query, ...props.query };
    for (const key in fillQuery) {
      if (!fillQuery[key as keyof QueryParams]) delete fillQuery[key as keyof QueryParams];
    }
    loaiXeFacade.get(fillQuery);
    navigate(
      { search: new URLSearchParams(fillQuery as unknown as Record<string, string>).toString() },
      { replace: true },
    );
    loaiXeFacade.set({ query: props.query, ...props.setKeyState });
  };

  const handleRemove = (id: string) => {
    modalApi.confirm({
      title: 'Xóa loại xe này ?',
      content: 'Mọi dữ liệu về loại xe này sẽ bị xóa vĩnh viễn. Bạn có chắc muốn xóa loại xe này ?',
      onOk: () => {
        loaiXeFacade.delete(id);
        loaiXeFacade.set({ isLoading: true });
        setTimeout(() => {
          onChangeDataTable({});
        }, 500);
      },
      onCancel: () => {},
      cancelText: 'Huỷ bỏ',
      okText: 'Xác nhận',
    });
  };

  const handleEdit = (data: LoaiXeModel) => {
    if (data.id) {
      setSearchParams(
        (prev) => {
          if (!prev.has('id')) prev.append('id', data.id ?? '');
          else prev.set('id', data.id ?? '');
          return prev;
        },
        { replace: true },
      );
      loaiXeFacade.set({ isEdit: true, isVisible: true, data: data, isFormLoading: true });
    }
  };

  const tool = (
    <Space>
      <SearchWidget form={(form) => (formRef.current = form)} callback={onChangeSearch} />
      <Button
        icon={<ReloadOutlined />}
        loading={loaiXeFacade.isLoading}
        onClick={() => {
          if (filter) {
            loaiXeFacade.get({ filter: filter });
          }
          loaiXeFacade.get({});
        }}
      >
        Tải lại
      </Button>

      {!rightMapRoleFacade?.rightDatas?.[0]?.rightCodes?.includes('DELETE') ? (
        <Tooltip title="Bạn không có quyền thực hiện thao tác này">
          <Button
            danger
            icon={<DeleteOutlined />}
            onClick={handleRemoveSelected}
            disabled={
              !rightMapRoleFacade?.rightDatas?.[0]?.rightCodes?.includes('DELETE') ||
              !loaiXeFacade.selectedRowKeys?.length
            }
          >
            Xóa ({loaiXeFacade.selectedRowKeys?.length ?? 0})
          </Button>
        </Tooltip>
      ) : (
        <Button
          danger
          icon={<DeleteOutlined />}
          onClick={handleRemoveSelected}
          disabled={!loaiXeFacade.selectedRowKeys?.length}
        >
          Xóa ({loaiXeFacade.selectedRowKeys?.length ?? 0})
        </Button>
      )}

      {!rightMapRoleFacade?.rightDatas?.[0]?.rightCodes?.includes('ADD') ? (
        <Tooltip title="Bạn không có quyền thực hiện thao tác này">
          <Button
            disabled={true}
            type={'primary'}
            icon={<PlusOutlined />}
            onClick={() => loaiXeFacade.set({ isVisible: true, data: undefined, isEdit: false })}
          >
            Thêm mới loại xe
          </Button>
        </Tooltip>
      ) : (
        <Button
          type={'primary'}
          icon={<PlusOutlined />}
          onClick={() => loaiXeFacade.set({ isVisible: true, data: undefined, isEdit: false })}
        >
          Thêm mới loại xe
        </Button>
      )}
    </Space>
  );
  const dataSource: DataType[] =
    loaiXeFacade.pagination?.content.map((items, index) => ({
      key: uuidv4(),
      id: items.id,
      stt: (Number(searchParams.get('page') ?? 0) - 1) * Number(searchParams.get('size') ?? 0) + index + 1,
      // bienSoXe: items.bienSoXe ?? '-',
      // soKhung: items.soKhung ?? '-',
      // soMay: items.soMay ?? '-',
      // hangSanXuat: items.hangSanXuat ?? '-',
      // model: items.model ?? '-',
      // namSanXuat: items.namSanXuat ?? '-',
      // taiXe: items.taiXe ?? '-',
      // taiTrong: items.taiTrong ?? '-',
      // active: items.active,
      tenLoaiXe: items.tenLoaiXe ?? '-',
      moTa: items.moTa ?? '-',
    })) ?? [];

  const colums: ColumnsType<DataType> = [
    {
      width: 300,
      title: 'Tên loại xe',
      dataIndex: 'tenLoaiXe',
      key: 'tenLoaiXe',
    },
    {
      title: 'Mô tả',
      dataIndex: 'moTa',
      key: 'moTa',
    },
    // {
    //   width: 170,
    //   title: 'Số máy',
    //   dataIndex: 'soMay',
    //   key: 'soMay',
    // },
    // {
    //   width: 150,
    //   title: 'Hãng sản xuất',
    //   dataIndex: 'hangSanXuat',
    //   key: 'hangSanXuat',
    // },
    // {
    //   width: 160,
    //   title: 'Model',
    //   dataIndex: 'model',
    //   key: 'model',
    // },
    // {
    //   width: 160,
    //   title: 'Năm sản xuất',
    //   dataIndex: 'namSanXuat',
    //   key: 'namSanXuat',
    // },
    // {
    //   width: 140,
    //   title: 'Tải trọng',
    //   dataIndex: 'taiTrong',
    //   key: 'taiTrong',
    // },
    // {
    //   width: 200,
    //   title: 'Tên tài xế',
    //   dataIndex: 'taiXe',
    //   key: 'taixe',
    // },
    // {
    //   width: 160,
    //   title: 'Trạng thái',
    //   dataIndex: 'active',
    //   key: 'status',
    //   render: (status) => (
    //     <Badge color={status ? 'green' : 'gray'} text={status ? 'Đang hoạt động' : 'Ngừng hoạt động'} />
    //   ),
    // },
    {
      title: 'Thao tác',
      dataIndex: 'action',
      key: 'action',
      align: 'center',
      width: 150,
      render: (_, record) => (
        <Space size={'small'}>
          {!rightMapRoleFacade?.rightDatas?.[0]?.rightCodes?.includes('UPDATE') ? (
            <Tooltip title="Bạn không có quyền thực hiện thao tác này">
              <Button disabled={true} type={'link'} onClick={() => handleEdit(record)}>
                Sửa
              </Button>
            </Tooltip>
          ) : (
            <Button type={'link'} onClick={() => handleEdit(record)}>
              Sửa
            </Button>
          )}

          {!rightMapRoleFacade?.rightDatas?.[0]?.rightCodes?.includes('DELETE') ? (
            <Tooltip title="Bạn không có quyền thực hiện thao tác này">
              <Button disabled={true} danger onClick={() => handleRemove(record.id ?? '')} type="link">
                Xóa
              </Button>
            </Tooltip>
          ) : (
            <Button danger onClick={() => handleRemove(record.id ?? '')} type="link">
              Xóa
            </Button>
          )}
        </Space>
      ),
    },
  ];

  return (
    <div>
      {contextHolder}
      <SubHeader tool={tool} isVisible={false} />
      <LoaiXeForm />
      <QuanLyLoaiXeDetails />
      <div className={'p-3'}>
        <Spin spinning={loaiXeFacade.isLoading}>
          <Table
            dataSource={dataSource}
            columns={colums}
            pagination={false}
            rowSelection={rowSelection as any}
            rowKey={'id'}
            scroll={{ y: 'calc(100vh - 290px)', x: '300px' }}
          />
          <Pagination
            className={'flex justify-end py-1'}
            showSizeChanger
            showTitle={false}
            current={loaiXeFacade?.pagination?.page}
            pageSize={loaiXeFacade?.pagination?.size}
            total={loaiXeFacade?.pagination?.totalElements}
            pageSizeOptions={[20, 40, 60, 80]}
            showTotal={(total, range) => `${range[0]}-${range[1]} of ${total} items`}
            onChange={(page, pageSize) => onChangeDataTable({ query: { page: page, size: pageSize } })}
          />
        </Spin>
      </div>
    </div>
  );
};
export default QuanLyLoaiXeScreen;
