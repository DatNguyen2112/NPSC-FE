...
import { Button, Col, Divider, Flex, Form, Input, Modal, Row, Tooltip } from 'antd';
...
export default function CreateModal() {
  ...
  const handleSubmit = (values: CodeTypeManagement) => {
    codeTypeManagementFacade.post({ ...values, type: 'ORGANIZATION_STRUCTURE' });
  };
  ...
  return (
    <Modal
      ...
      centered
      open={codeTypeManagementFacade.isCreateCodeType}
      width={700}
      onCancel={() => codeTypeManagementFacade.set({ isCreateCodeType: false })}
      ...
      <Row gutter={14}>
        <Col xs={8}>
          <Form.Item name="code" label="Mã phân loại cha" rules={[{ required: true, message: 'Vui lòng chọn nhóm' }]}>
            <Input placeholder="Nhập mã phân loại cha" />
          </Form.Item>
        </Col>
        <Col xs={16}>
          <Form.Item
          ...
          >
            <Input placeholder="Nhập tên phân loại cha" />
          </Form.Item>
        </Col>
      </Row>
