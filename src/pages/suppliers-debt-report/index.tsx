...
import SupplierDebtGlossary from './SupplierDebtGlossary';
import SupplierDebtMinusModal from './SupplierDebtMinusModal';
import SupplierDebtPositiveModal from './SupplierDebtPositiveModal';
...
const SupplierDebtReport: React.FC = () => {
  ...
  const [supplierDebtForm] = Form.useForm();
  ...
  const minClosingDebtValue = supplierDebtForm.getFieldValue('minClosingDebt');
  const maxClosingDebtValue = supplierDebtForm.getFieldValue('maxClosingDebt');
  const positiveValue = supplierDebtForm.getFieldValue('positive') ?? true;
  ...
  const handleCloseTag = (e: React.MouseEvent<HTMLElement>) => {
    ...
    supplierDebtForm.setFieldsValue({
  ...
  const columns: ColumnsType<DebtTransactionReport> = [
    ...
    },
    {
      ...
      render: (entityCode: string, record) => (
        <Link
          to={{
            pathname: `/${lang}${routerLinks('NhaCungCap')}/${record.entityId}/view-detail`,
              ...
              <span
                ...
                onClick={() => {
                  debtTransactionFacade.set({
                    isDebtReportModalPositiveSupplierVisible: true,
                    supplierId: record.entityId,
                  });
                }}
              >
          ...
          <>
            {debtDecrease > 0 ? (
              <span
                ...
                onClick={() => {
                  debtTransactionFacade.set({
                    isDebtReportModalNegativeSupplierVisible: true,
                    supplierId: record.entityId,
                  });
                }}
              >
    ...
    <>
      <Spin spinning={debtTransactionFacade.isLoading}>
        <div className="px-8 pb-10">
          <div className="py-3 flex justify-between">
            ...
            <div>
              <Button
                ...
                type="text"
                onClick={() => debtTransactionFacade.set({ isDebtReportModalGlossarySupplierVisible: true })}
              >
                            ...
                            >
                              <Row gutter={[0, 0]}>
                                <Col span={11}>
                                  <Form.Item
                                    ...
                                    rules={[
                                      {
                                        validator: (_, value) =>
                                          value && supplierDebtForm.getFieldValue('maxClosingDebt') < value
                                  ...
                                  >
                                    <InputNumber
                                      ...
                                      onChange={(minClosingDebt) => {
                                        if (minClosingDebt) {
                                          supplierDebtForm.setFieldsValue({ positive: false });
                                        }
                                      }}
                                    />
                                  </Form.Item>
                                ...
                                <Col span={11}>
                                  <Form.Item
                                    ...
                                    rules={[
                                      {
                                        validator: (_, value) =>
                                          value && supplierDebtForm.getFieldValue('minClosingDebt') > value
                                            ? Promise.reject('Giá trị không đúng định dạng')
                                            : Promise.resolve(),
                                      },
                                    ]}
                                ...
                                <Col className="pt-3" span={24}>
                                  <Button className="w-full" type="primary" onClick={supplierDebtForm.submit}>
                                    Lọc
                                  </Button>
                                </Col>
                              </Row>
          ...
          {debtTransactionFacade.isDebtReportModalGlossarySupplierVisible && <SupplierDebtGlossary />}
          {debtTransactionFacade.isDebtReportModalPositiveSupplierVisible && debtTransactionFacade.supplierId && (
            <SupplierDebtPositiveModal supplierId={debtTransactionFacade.supplierId} />
          )}
          {debtTransactionFacade.isDebtReportModalNegativeSupplierVisible && debtTransactionFacade.supplierId && (
            <SupplierDebtMinusModal supplierId={debtTransactionFacade.supplierId} />
          )}
        </div>
      </Spin>
    </>
