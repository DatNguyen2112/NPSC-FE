import { CaretDownOutlined, ExportOutlined, FilterFilled, PlusOutlined, ReloadOutlined } from '@ant-design/icons';
...
import { Button, Dropdown, FormInstance, Modal, Pagination, Select, Space, Spin, Table, Tabs, Tag } from 'antd';
...
import ExportFileReceiptVouchersModal from './ExportFileModal';
import FilterModal from './filter.drawer';
import './index.less';
...
const Page: React.FC = () => {
  ...
  const hasSelected = cashBookTransactionFacade.selectedRowKeysReceiptVoucher?.length ?? 0 > 0;
  ...
  if (filter) {
    ...
    filterObject = { transactionTypeCode: 'THU'};
  }

  useEffect(() => {
    cashBookTransactionFacade.set({ activeKey: 'all', selectedRowKeysReceiptVoucher: [], dateRange: undefined });
  ...
  const onChangeSearch = (value: string) => {
    ...
    if (filter) {
      filterObject = JSON.parse(filter);
      filterObject.FullTextSearch = value;
  ...
  const rowSelection = {
    onChange: (selectedRowKeys: string[]) => {
      cashBookTransactionFacade.set({ selectedRowKeysReceiptVoucher: selectedRowKeys });
    },
  ...
  const handleCancelVoucherCashbookTransaction = () => {
    modalApi.confirm({
      width: 600,
      title: `Bạn chắc chắn muốn huỷ ${cashBookTransactionFacade.selectedRowKeysReceiptVoucher?.length} phiếu thu này?`,
      content: 'Thao tác này sẽ huỷ các phiếu thu bạn đã chọn. Thao tác này không thể khôi phục.',
      onOk: () => {
        if (cashBookTransactionFacade.selectedRowKeysReceiptVoucher?.length) {
          cashBookTransactionFacade.cancelVoucherCashbookTransactions(
            cashBookTransactionFacade.selectedRowKeysReceiptVoucher,
          );
        }
      },
      onCancel: () => {},
  ...
  const handleEdit = (id?: string) => {
    if (id) {
      navigate(`/${lang}${routerLinks('PhieuThu')}/${id}/edit`);
    }
  };

  const handleCreate = () => {
    cashBookTransactionFacade.set({ dataCashbookTransaction: {} });
    navigate(`/${lang}${routerLinks('PhieuThu')}/create`);
  };

  const handleOpenExportFileModal = () => {
    cashBookTransactionFacade.set({ isExportFileModalReceiptVoucher: true });
  };
  ...
  const handleMenuClick: MenuProps['onClick'] = (e) => {
    switch (e.key) {
      case 'normal':
        ...
        break;
      case 'multiple':
        navigate(`/${lang}${routerLinks('PhieuThu')}/create-multiple`);
        break;
    }
  };
    ...
    },
    {
      ...
      render: (code: string, record: CashbookTransactionModel) => (
        <Tooltip title={'Xem chi tiết phiếu thu'}>
          <Link className="hover:underline" to={`/${lang}${routerLinks('PhieuThu')}/${record.id}/edit`}>
            {code}
          </Link>
        </Tooltip>
      ),
              ...
              items: [
                {
                  ...
                  label: (
                    <a onClick={() => handleEdit(record.id)} className="text-gray-900 hover:!text-blue-500">
                      Chỉnh sửa
                    </a>
                  ),
                },
  ...
  const items: TabsProps['items'] = [
    {
      key: 'all',
      label: <h1 className="pl-3">Tất cả phiếu thu</h1>,
      children: (
        <>
          <div className="flex items-center gap-3 p-4">
            <div className="flex-1">
              <SearchWidget
                tabActive={cashBookTransactionFacade.activeKey}
                form={(form) => (formRef.current = form)}
                callback={onChangeSearch}
                placeholder={'Tìm kiếm theo mã phiếu thu, tham chiếu, loại phiếu '}
              />
            </div>
            <div className={'flex gap-2.5'}>
              ...
              <Button
                ...
                icon={<FilterFilled />}
                iconPosition="end"
                onClick={() => cashBookTransactionFacade.set({ isFilter: true })}
              >
                Bộ lọc khác
              </Button>
            </div>
            ...
            <Table
              title={() => (
                <>
                  <Space size={'small'}>
                    <p>
                      Đã chọn {cashBookTransactionFacade.selectedRowKeysReceiptVoucher?.length} phiếu thu trên trang này
                    </p>
                    <Button
                      className="font-medium text-red-500"
                      type="link"
                      onClick={handleCancelVoucherCashbookTransaction}
                    >
                      Hủy phiếu thu
                    </Button>
                  </Space>
    ...
    {
      key: 'COMPLETED',
      label: <h1>Phiếu thu hoàn thành</h1>,
      children: (
        <>
          <div className="flex items-center gap-3 p-4">
            <div className="flex-1">
    ...
    },
    {
      ...
      children: (
        <>
          ...
          <div className={'mb-2 mx-3.5 flex items-center gap-2 flex-wrap'}>
            {filterObject.FullTextSearch && true && (
              <Tag
                closable
                onClose={() => {
              ...
              title={() => (
                <>
                  <Space size={'small'}>
                    ...
                    <Button className="font-medium" type="link" onClick={handleCancelVoucherCashbookTransaction}>
                      Hủy phiếu thu
                    </Button>
                  </Space>
                </>
  ...
  return (
    <>
      ...
      <Spin spinning={cashBookTransactionFacade.isLoading}>
        ...
        <ExportFileReceiptVouchersModal />
      </Spin>
      <FilterModal />
    </>
  );
