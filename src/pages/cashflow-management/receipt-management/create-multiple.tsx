...
import {
  ...
  ContractFacade,
  ConstructionFacade, CashbookTransactionCreateModel, EStatusThuChi, UserFacade, UserModal,
} from '@store';
...
const ReceiptCreateMultiplePage = () => {
  ...
  const entityTypeCode = receiptCreateForm.getFieldValue('listCashbookTransaction');
    ...
    listCashbookTransaction = listCashbookTransaction.map((items: any, index: number) => (
      {
        ...
        entityCode:
          items.entityTypeCode === 'customer' && customerFacade.pagination?.content.find((item) => item.id === items.entityId)?.code
          || items.entityTypeCode === 'supplier' && supplierFacade.pagination?.content.find((item) => item.id === items.entityId)?.code
          || items.entityTypeCode === 'employee' && userFacade.pagination?.content.find((item) => item.id === items.entityId)?.userName
          || entityTypeCode[index]?.entityTypeCode,
      }
    ));

    const receiptVoucherData: CashbookTransactionCreateModel = {
      listCashbookTransaction: listCashbookTransaction,
      constructionId: values.constructionId,
      contractId: values.contractId,
    };
    if (values) {
      cashBookTransactionFacade.createMultiple(receiptVoucherData);
    }
  };

  const handleAdd = () => {
    const listCashbookTransaction = receiptCreateForm.getFieldValue('listCashbookTransaction');
          ...
          >
            <Row gutter={16}>
              <Col span={24}>
                ...
                {/* Thông tin ghi nhận */}
                <Card title="Thông tin phiếu thu" className="mt-4">
                            ...
                            <Table.Column
                              ...
                              title={<div className={'flex gap-1 items-center'}>
                                ...
                                <Tooltip
                                  placement="top"
                                  trigger={'hover'}
                                  title={`Mã phiếu thu không trùng lặp. Nếu để trống mã phiếu tự sinh với tiền tố RVN`}>
                                      ...
                                      <Select
                                        ...
                                        onChange={(value) => {
                                          ...
                                          switch (value) {
                                            ...
                                            case 'employee':
                                              userFacade.get({ size: -1 });
                                              cashBookTransactionFacade.set({ placeholderEntityId: 'Chọn nhân viên' });
                                              break;
                                          }
                                        }}
                                      />
                              ...
                              render={(_, _record: any, index: number) => {
                                return (
                                  <>
                                    <Form.Item
                                      rules={[{ required: true }]}
                                      name={entityTypeCode[index]?.entityTypeCode === 'employee' ?  [index, 'entityName'] : [index, 'entityId']}
                                    >
                                      {
                                        entityTypeCode[index]?.entityTypeCode === 'other' ? (
                                          ...
                                          <Select
                                            ...
                                            options={
                                              ...
                                              || entityTypeCode[index]?.entityTypeCode === 'employee' && userFacade.pagination?.content.map((item: UserModal) => {
                                                return {
                                                  label: item.name,
                                                  value: item.id,
                                                  phoneNumber: item.phoneNumber,
                                      ...
                                      rules={[
                                        {
                                          required: true,
                                          message: <span
                                            className={'text-sm]'}>Loại phiếu thu không được để trống</span>,
                                        },
                                      ]}
                                    >
                                      <Select
                              ...
                              title={
                                <div className={'flex gap-1 justify-center'}>
                                  ...
                                  <Tooltip
                                    ...
                                    title={
                                      <p className="text-sm">
                                        Nếu không chọn, giá trị của phiếu thu không được tính vào công nợ của đối tượng
                                        nộp.
                                      </p>
                                    }
                                  >
...
export default ReceiptCreateMultiplePage;
